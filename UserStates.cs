...
public sealed class UserStates {
    ...
    public static async Task Initialize(DiscordClient client) {
        Log.Information("Initalizing UserStates.");
        ...
        try {
            ...
            if (d == null) {
                Log.Error("Could not deserialize balances on disk.");  
            } else {
                _instance._balances = d;
            }
        } catch (FileNotFoundException) {
            Log.Error("Could not find balances on disk.");
        }
        Log.Information("Finished initalizing UserStates.");
    }

    public void StoreState() {
        Log.Information("Storing UserStates to disk.");
        lock(_balanceLock)
